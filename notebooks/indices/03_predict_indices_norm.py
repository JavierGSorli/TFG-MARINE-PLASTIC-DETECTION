...
def minmax_normalize_per_band(img_chw: np.ndarray) -> np.ndarray:
    """
    Normaliza cada banda de forma independiente a [0, 1] usando MinMax.
    """
    out = np.full_like(img_chw, np.nan, dtype=np.float32)
    for b in range(img_chw.shape[0]):
        band = img_chw[b].astype(np.float32, copy=False)
        finite = np.isfinite(band)
        if not np.any(finite):
            continue

        vmin = float(np.min(band[finite]))
        vmax = float(np.max(band[finite]))
        if vmax <= vmin:
            out[b, finite] = 0.0
        else:
            out[b, finite] = (band[finite] - vmin) / (vmax - vmin)
    return out


def compute_fdi_ndvi(img_chw: np.ndarray):
    """
    img_chw: (11, H, W) float32
    """
    b04 = img_chw[3]   # red
...
def process_one(tif_path: Path, out_dir: Path):
    ...
    # Cambio respecto a 03_predict_indices.py:
    # normalización MinMax por banda antes de calcular índices.
    img_norm = minmax_normalize_per_band(img)
    fdi, ndvi = compute_fdi_ndvi(img_norm)
...
def main():
    ...
    csv_path = out_dir / "summary_indices_norm.csv"
